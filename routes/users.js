...
router.get('/read/:maccAddress/', async function(req, res, next) {
  try {
    if(req.query){
      const macAddress = req.params.macAddress
      const chip = req.params.chip
      const url = 'http://192.168.1.200/gatt/nodes/'+macAddress+'/handle/'+req.query.handle+'/value?';
      // const sse = new EventSource(url);
      const headers =  {
        'Content-Type': 'application/json'
      }
...
router.post('/write/:type', async function(req, res, next) {
  try {
    if(req.body.macAddress && req.body.handle && req.body.value){
      ...
      const url = 'http://192.168.1.200/gatt/nodes/'+macAddress+'/handle/' +handle + '/value/' + value ;
      // const sse = new EventSource(url);
      const headers =  {
        'Content-Type': 'application/json'
      }
...
router.get('/delete/:macAddress', async function(req, res, next) {
  try {
    if(req.params.macAddress){
      const macAddress = req.params.macAddress
      const url = 'http://192.168.1.200/gap/nodes/'+macAddress+'/connection?' ;
      // const sse = new EventSource(url);
      const headers =  {
        'Content-Type': 'application/json'
      }
...
router.get('/getDeviceConnected', async function(req, res, next) {
  try {
      const url = 'http://192.168.1.200/gap/nodes?&connection_state=connected';
      const headers =  {
        'Content-Type': 'application/json'
      }
      const connection = await axios.get(url, headers)
...
router.get('/deviceServices/:macAddress', async function(req, res, next) {
  try {
    const macAddress = req.params.macAddress
    const url = 'http://192.168.1.200/gatt/nodes/'+macAddress+'/services/characteristics/descriptors?';
    const headers =  {
      'Content-Type': 'application/json'
    }
    const connection = await axios.get(url, headers)
